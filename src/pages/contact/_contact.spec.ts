...
test.describe('お問い合わせページ', () => {
  ...
  test('カタカナ以外のふりがなが入力されるとボタンが無効', async ({ page }) => {
    ...
    for (const kana of invalidKanaInputs) {
      ...
      confirmBtnIsDisabled(confirmBtn);
    }
  });

  test('無効なメールアドレスが入力されるとボタンが無効', async ({ page }) => {
    const invalidEmailInputs = ['invalidEmail', 'plainaddress', '@missingusername.com', 'missingat.com', 'user@domain', 'user@domain.'];
 ...
 * フォームに入力する共通関数
 */
const fillForm = async (page: Page, { name = '佐藤花子', kana = 'サトウハナコ', email = 'hanako.sato@example.com', message = 'playwrightによる自動e2eテストを実施しています。', agreePrivacy = true }: { name?: string; kana?: string; email?: string; message?: string; agreePrivacy?: boolean }) => {
