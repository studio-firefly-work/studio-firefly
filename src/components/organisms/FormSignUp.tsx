...
export default function FormSignUp() {
  ...
  const onSubmit = async (data: FormSignUpDataType) => {
    if (!isConfirming) {
      ...
      try {
        const res = await fetch(`${import.meta.env.PUBLIC_API}/user/`, {
          method: 'POST',
          headers: {
    ...
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmit)}>
        {!isConfirming ? (
          ...
          <FormSignUpConfirm setIsConfirming={setIsConfirming} />
        )}
      </form>
    </FormProvider>
  )
...
const FormSignUpInput = () => {
  ...
  return (
    <div className='flex flex-col gap-4'>
      <div>
        <label htmlFor='email-sign-up' className='label-text'>メールアドレス</label>
      ...
      <div>
        <label htmlFor='password-sign-up' className='label-text'>パスワード</label>
